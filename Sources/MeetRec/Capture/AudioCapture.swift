...
func captureFromDevice(_ deviceID: AudioDeviceID) throws -> AsyncStream<CaptureEvent> {
  ...
  input.installTap(onBus: 0, bufferSize: 4096, format: format) { buffer, when in
    ...
    continuation.yield(
      .bytes(data: rawBytes(of: buffer), pts: microseconds(fromHostTime: hostTime)))
  }
  ...
  let configObserver = NotificationCenter.default.addObserver(
    ...
    continuation.yield(
      .closed(CaptureError("The microphone was disconnected or its configuration changed.")))
    continuation.finish()
  }
...
private func rawBytes(of buffer: AVAudioPCMBuffer) -> [UInt8] {
  ...
  for audioBuffer in UnsafeMutableAudioBufferListPointer(buffer.mutableAudioBufferList) {
    ...
    bytes.append(
      contentsOf: UnsafeRawBufferPointer(start: data, count: Int(audioBuffer.mDataByteSize)))
  }
  return bytes
}
...
private func processObject(forPID pid: pid_t) -> AudioObjectID? {
  ...
  let status = AudioObjectGetPropertyData(
    AudioObjectID(kAudioObjectSystemObject), &address, UInt32(MemoryLayout<pid_t>.size), &pidVar,
    &dataSize, &objectID
  )
